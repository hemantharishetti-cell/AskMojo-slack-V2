...
- **Vector Search**: ChromaDB + OpenAI `text-embedding-3-small` for semantic search over document chunks
...
| **openai embeddings** | Embedding model (`text-embedding-3-small`, 1536 dims) for semantic search |
...
2. **Processing**: Background task (after `VECTOR_PROCESSING_DELAY` seconds) chunks PDF with `pdfplumber`, generates embeddings with OpenAI `text-embedding-3-small`, and stores them in ChromaDB.
