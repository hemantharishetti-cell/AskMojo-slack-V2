...
async def process_slack_message(
    ...
    # ── Call app.dev_v2 agent directly ─────────────────────────────
    try:
        from app.dev_v2 import run_agent
        import json as _json

        agent_result_raw = run_agent(question)
        
        try:
            agent_data = _json.loads(agent_result_raw) if isinstance(agent_result_raw, str) else agent_result_raw
            answer_text = agent_data.get("answer", "I'm sorry, I couldn't find an answer to that question.")
        except Exception:
            answer_text = "Sorry, I received an invalid response from the agent engine."

    except Exception as e:
        logger.error("Deep Agent error: %s", e, exc_info=True)
