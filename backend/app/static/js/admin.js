    ...
    try {
        ...
        if (window.adminSearchQuery) {
            ...
            users = users.filter(user =>
                ...
                (user.id && String(user.id).includes(query))
            );
        }

        if (users.length === 0) {
            tbody.innerHTML = `<tr class="table-row"><td colspan="7" class="px-8 py-16 text-center text-gray-500"><i class="fas fa-users text-3xl mb-4 block text-gray-400"></i><p class="font-medium">${window.adminSearchQuery ? 'No matches found for "' + window.adminSearchQuery + '"' : 'No users found'}</p></td></tr>`;
            return;
        }
...
function showNotification(message, type = 'info') {
    ...
    notification.className = `fixed top-4 right-4 px-6 py-4 rounded-lg shadow-lg z-50 ${type === 'success' ? 'bg-green-500' :
