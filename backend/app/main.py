...
from app.vector_logic.vector_store import init_chromadb, warm_reranker
...
# Add CORS middleware
# Allowed origins are set via CORS_ORIGINS in .env — never use ["*"] in production
app.add_middleware(
    CORSMiddleware,
...
async def on_startup():
    ...
    logger.info("Warming reranker model...")
    if warm_reranker():
        logger.info("[OK] Reranker ready")
    else:
        logger.warning("Reranker warm-up failed - first request may incur cold-start latency")
...
async def on_shutdown():
    ...
    # Stop Slack Socket Mode client first to avoid background threads
    # submitting work during interpreter shutdown.
    try:
        from app.slack.socket_mode import stop_socket_mode_client
        stop_socket_mode_client()
        logger.info("[OK] Slack Socket Mode stopped")
    except Exception as e:
        logger.warning("Error stopping Slack Socket Mode: %s", e)
...
# Global exception handler (last-resort): never expose internals
@app.exception_handler(Exception)
async def _global_exception_handler(request: Request, exc: Exception):
    try:
        # Best-effort capture of user and query
        user = getattr(request.state, "user_email", None) or "system"
