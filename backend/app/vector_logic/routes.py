...
from app.vector_logic.description_generator import refine_description
...
from monitoring.error_handler import handle_error  # NEW: centralized error monitoring
...
def verify_grounding(answer: str, source_chunks: list, client, model="gpt-4o-mini") -> tuple[bool, str]:
    """
    Checks if the generated answer contains ungrounded claims (hallucinations).
    Returns (is_grounded, corrected_answer).
    """
    if not answer or not source_chunks:
        return True, answer
        
    chunks_text = "\n\n".join([f"Source {i+1}:\n{c.chunk_text}" for i, c in enumerate(source_chunks)])
    
    prompt = f"""You are a strict fact-checker for a RAG system.
Evaluate if the following ANSWER is fully grounded in the provided SOURCE_CHUNKS.

1. Identify any claims in the ANSWER that cannot be directly proven by the SOURCE_CHUNKS.
2. If the answer contains ungrounded claims, rewrite it to remove them, keeping the original tone.
3. If the answer is completely ungrounded, return "I could not find information to support this."
...
"""
    try:
        ...
        return res.get("is_grounded", True), res.get("corrected_answer", answer)
    except Exception as e:
        print(f"[GROUNDING] Error verifying: {e}")
        return True, answer
...
def get_description_text(description_raw: str | None) -> str:
    """
    Safely extract searchable text from a description (JSON or plain text).
    If JSON, joins all values into a single string.
    """
    if not description_raw:
        return ""
    try:
        data = json.loads(description_raw)
...
def upload_document(
    ...
    domain_id: int | None = Form(None),  # Domain ID from Domain table (optional)
    category: str | None = Form(None),  # Legacy: kept for backward compatibility
    description: str | None = Form(None),  # Optional description (if provided, will be used; otherwise generated during background OCR processing)
    ...
    """
    try:
        # Check for duplicate file name in the same domain (or globally if no domain)
        ...
        # -----------------------------
        # 2.5 Validate domain_id (optional)
        # -----------------------------
        if domain_id is not None:
        ...
        # -----------------------------
        # 3. Use original filename (Secured)
        # -----------------------------
        filename_base = Path(file.filename).stem
        
        # Check for collisions and add suffix only if needed
        safe_filename = f"{filename_base}{ext}"
        file_path = UPLOAD_DIR / safe_filename
        
        if file_path.exists():
            safe_filename = f"{filename_base}_{uuid4().hex[:4]}{ext}"
            file_path = UPLOAD_DIR / safe_filename
        ...
        # -----------------------------
        # 5. Create basic DB record first
        # -----------------------------
        source_type_map = {
            '.pdf': 'pdf',
            '.doc': 'doc',
        ...
        document = Document(
            ...
            domain_id=domain_id,
            description=(description or "") or "Processing (OCR pending)",
        ...
        db.commit()
        db.refresh(document)
        print("Document saved to SQL lite successfully")

        # -----------------------------
        # 6. Create upload log entry (processing will generate description later)
        # -----------------------------
        import time
        ...
        upload_log = DocumentUploadLog(
            ...
            description_generation_time_seconds=None,
            description_tokens_used=None,
            description_tokens_prompt=None,
            description_tokens_completion=None
        )
        db.add(upload_log)
        db.commit()
...
def ask_question(
    ...
    try:
        ...
        # [FIX MAJ-01] Initialize variables with safe defaults to prevent UnboundLocalError
        # or NameError in downstream except blocks.
        token_usage_summary = {}
        ...
        # INTENT_ROUTER_STAGE: Isolate classification to monitor failures
        try:
            intent, intent_hints = classify_intent(
                request.question, 
                openai_api_key=settings.openai_api_key
            )
            ...
            handle_error(
                stage="INTENT_ROUTER_STAGE",
                user=request.slack_user_email or "api",
...
{{
    ...
    "answer_mode": "<extract|brief|summarize|explain>"
        ...
        # VECTOR_STORE_STAGE: Master collection query
        try:
        ...
        if preferred_solution:
            ...
            for doc_id in filtered_documents:
                ...
                if not doc:
                    continue
                desc_text = get_description_text(doc.description)
                text = ((doc.title or "") + " " + desc_text).lower()
       ...
       #     )
        ...
        if 'filtered_documents' in locals() and filtered_documents:
            ...
            for collection_name, doc_ids_in_collection in docs_by_collection.items():
                ...
                try:
                    from app.vector_logic.vector_store import query_bm25, reciprocal_rank_fusion
                    print(f"  Collection '{collection_name}': Hybrid (BM25+Dense) query for {len(doc_ids_in_collection)} docs (limit={query_limit_global})")
                    
                    # 1. DENSE RETRIEVAL (ChromaDB)
                    dense_raw = query_collection_with_filter(
                        query_text=refined_question,
                        collection_name=collection_name,
                    ...
                    dense_chunks = []
                    if dense_raw.get("ids") and dense_raw["ids"][0]:
                        for chunk_idx, chunk_id in enumerate(dense_raw["ids"][0]):
                            c_meta = dense_raw["metadatas"][0][chunk_idx] if dense_raw.get("metadatas") else {}
                            dense_chunks.append({
                                "id": chunk_id,
                                "document_id": c_meta.get("document_id"),
                                "chunk_text": dense_raw["documents"][0][chunk_idx] if dense_raw.get("documents") else "",
                                "page_number": c_meta.get("page_number"),
                                "chunk_index": c_meta.get("chunk_index"),
                                "score": float(dense_raw["distances"][0][chunk_idx]) if dense_raw.get("distances") else 0.0
                            })

                    # 2. SPARSE RETRIEVAL (BM25)
                    sparse_raw = query_bm25(
                    ...
                    merged_chunks = reciprocal_rank_fusion(dense_chunks, sparse_raw, id_key="id")

                    # 4. PROCESS RESULTS AND DISTRIBUTE TO DOCUMENTS
                    chunks_found_map = defaultdict(int)
                    for chunk_dict in merged_chunks:
                        chunk_id_str = chunk_dict.get("id", "")
                        chunk_doc_id = chunk_dict.get("document_id")

                        # If sparse chunk didn't have metadata, extract doc_id from id string (e.g., "doc_123_chunk_4")
                        if chunk_doc_id is None and chunk_id_str.startswith("doc_"):
                            try:
                                chunk_doc_id = int(chunk_id_str.split("_")[1])
                        ...
                        if chunk_doc_id in doc_ids_in_collection:
                            ...
                            if chunks_found_map[chunk_doc_id] < doc_limits.get(chunk_doc_id, 10):
                                ...
                                # Use dense score if available, else a neutral placeholder
                                # so downstream RERANKER and data quality checks don't break.
                                distance_score = chunk_dict.get("score", 0.5)
                                
                                # Extract chunk_index if missing
                                c_idx = chunk_dict.get("chunk_index")
                                if c_idx is None and "_chunk_" in chunk_id_str:
                                    try:
                                        c_idx = int(chunk_id_str.split("_chunk_")[1])
                                    except (IndexError, ValueError):
                                        pass
                    ...
                    # VECTOR_STORE_STAGE: Per-collection chunk retrieval
                    try:
                        handle_error(
                            stage="VECTOR_STORE_STAGE",
                            user=request.slack_user_email or "api",
        ...
        if source_chunks and len(source_chunks) > 3:
            try:
                from app.vector_logic.vector_store import rerank_chunks
                
                print(f"  [RERANKER] Re-ranking {len(source_chunks)} combined chunks...")
                rerank_start = time.time()
                
                # Determine top_k dynamically based on answer mode
                if answer_mode in ("extract", "brief"):
                    rerank_top_k = min(8, len(source_chunks))
                elif answer_mode == "summarize":
                    rerank_top_k = min(15, len(source_chunks))
                else:  # explain
                    rerank_top_k = min(10, max_chunks_allowed)
                
                source_chunks = rerank_chunks(
                    query=refined_question,
                ...
                # Re-ranker returns sorted by score. The cross-encoder score is entirely different
                # from ChromaDB's cosine distance, so we don't overwrite chunk.score to maintain
                # compatibility with downstream distance checks (relevance_quality), though we could.
                
                print(f"  [RERANKER] Done in {time.time() - rerank_start:.2f}s. Kept top {len(source_chunks)} chunks.")
                if source_chunks:
                     print(f"  [RERANKER] Top chunk: '{source_chunks[0].chunk_text[:60]}...'")
            except Exception as e:
                print(f"  [RERANKER] Error during re-ranking: {e}")
...
"""
    ...
    Never reveal financial data. Always cite document titles, never page numbers."""
        ...
        # ── FIX 6: Grounding-based hallucination guard ────────────────────────
        try:
            print("  [GROUNDING] Running post-generation hallucination check...")
            grounding_start = time.time()
            is_grounded, grounded_answer = verify_grounding(
                answer=answer, 
                source_chunks=source_chunks, 
                client=client, 
                model="gpt-4o-mini"
            )
            print(f"  [GROUNDING] Check complete in {time.time() - grounding_start:.2f}s. Passed: {is_grounded}")
            
            if not is_grounded:
                print(f"  [GROUNDING] Hallucinations detected! Overwriting answer with grounded version.")
                answer = grounded_answer
        except Exception as e:
            print(f"  [GROUNDING] Error during check: {e}")
        # ──────────────────────────────────────────────────────────────────────
        ...
        # ROUTE_PROCESS_STAGE: Global catch for this route
        try:
            handle_error(
                stage="ROUTE_PROCESS_STAGE",
                user=(request.slack_user_email if 'request' in locals() and getattr(request, 'slack_user_email', None) else "api"),
                query=(request.question if 'request' in locals() and getattr(request, 'question', None) else ""),
                error=e,
                severity="CRITICAL",
            )
        except Exception:
