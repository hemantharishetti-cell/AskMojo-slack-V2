...
@router.get("/socket-mode/status", response_model=SlackSocketModeStatusResponse)
def get_socket_mode_status(current_user=Depends(get_current_admin_user)):
    """Get runtime Slack Socket Mode connection status."""
    return get_socket_mode_runtime_status()
            ...
            async with httpx.AsyncClient() as client:
                try:
                    ...
                    request_data = {
                        ...
                        "request_user_key": f"slack:{user_id.lower()}" if user_id else None,
                    }
                    if slack_user_email:
                    ...
                    # Monitor Slack integration errors but keep user-facing response
                    try:
                        handle_error(
                            stage="SLACK_STAGE",
                            user=slack_user_email or user_id or "slack",
                            query=question_text,
                            error=e,
                            severity="ERROR",
                        )
                    ...
                    if isinstance(e, httpx.HTTPStatusError):
                        try:
                            response_text = e.response.json().get("detail")
                        except Exception:
                            response_text = None
                    response_text = response_text or f"Sorry, I encountered an error processing your question: {str(e)}"
