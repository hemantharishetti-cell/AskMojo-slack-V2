...
class DocumentUploadLog(Base):
    ...
    document_description = Column(String, nullable=True)  # The generated description text
    chunk_count = Column(Integer, nullable=True)  # Number of extracted chunks
    ...
    processing_stage = Column(String, nullable=True)  # Current processing stage (e.g., OCR_PIPELINE, VECTOR_STORAGE)
    created_at = Column(DateTime, default=datetime.utcnow, nullable=False)
...
class SlackUser(Base):
    ...
    is_processing = Column(Boolean, default=False, nullable=False)  # Whether this user currently has an active query
    processing_started_at = Column(DateTime, nullable=True)  # When the current query lock was acquired
    processing_request_id = Column(String, nullable=True)  # Request id that currently owns the lock
    created_at = Column(DateTime, default=datetime.utcnow, nullable=False)
    updated_at = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow, nullable=False)
