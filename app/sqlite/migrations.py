...
def migrate_slack_users_table():
    ...
    existing_columns = [col["name"] for col in inspector.get_columns("slack_users")]

    with engine.connect() as conn:
        if "is_processing" not in existing_columns:
            print("Adding 'is_processing' column to slack_users table...")
            conn.execute(text("ALTER TABLE slack_users ADD COLUMN is_processing INTEGER NOT NULL DEFAULT 0"))
            conn.commit()
            print("[OK] Added 'is_processing' column")

        if "processing_started_at" not in existing_columns:
            print("Adding 'processing_started_at' column to slack_users table...")
            conn.execute(text("ALTER TABLE slack_users ADD COLUMN processing_started_at DATETIME"))
            conn.commit()
            print("[OK] Added 'processing_started_at' column")

        if "processing_request_id" not in existing_columns:
            print("Adding 'processing_request_id' column to slack_users table...")
            conn.execute(text("ALTER TABLE slack_users ADD COLUMN processing_request_id VARCHAR"))
            conn.commit()
            print("[OK] Added 'processing_request_id' column")
...
def migrate_document_upload_logs_table():
    ...
    with engine.connect() as conn:
        ...
        # Add processing-related fields
        if "processing_stage" not in existing_columns:
            print("Adding 'processing_stage' column to document_upload_logs table...")
            conn.execute(text("ALTER TABLE document_upload_logs ADD COLUMN processing_stage VARCHAR"))
            conn.commit()
            print("[OK] Added 'processing_stage' column")

        if "processing_started" not in existing_columns:
            print("Adding 'processing_started' column to document_upload_logs table...")
            conn.execute(text("ALTER TABLE document_upload_logs ADD COLUMN processing_started INTEGER NOT NULL DEFAULT 0"))
            conn.commit()
            print("[OK] Added 'processing_started' column")

        if "processing_completed" not in existing_columns:
            print("Adding 'processing_completed' column to document_upload_logs table...")
            conn.execute(text("ALTER TABLE document_upload_logs ADD COLUMN processing_completed INTEGER NOT NULL DEFAULT 0"))
            conn.commit()
            print("[OK] Added 'processing_completed' column")

        if "processing_error" not in existing_columns:
            print("Adding 'processing_error' column to document_upload_logs table...")
            conn.execute(text("ALTER TABLE document_upload_logs ADD COLUMN processing_error VARCHAR"))
            conn.commit()
            print("[OK] Added 'processing_error' column")

        if "processed_at" not in existing_columns:
            print("Adding 'processed_at' column to document_upload_logs table...")
            conn.execute(text("ALTER TABLE document_upload_logs ADD COLUMN processed_at DATETIME"))
            conn.commit()
...
def migrate_domain_required_triggers():
    """Remove strict domain triggers as domain is now optional."""
    inspector = inspect(engine)
    ...
    with engine.connect() as conn:
        if "documents" in tables:
            try:
                ...
                conn.commit()
                print("[OK] Removed documents.domain_id triggers")
            except Exception as e:
                print(f"[WARN] Could not drop documents domain triggers: {e}")
        ...
        if "document_upload_logs" in tables:
            try:
                ...
                conn.commit()
                print("[OK] Removed document_upload_logs.domain_id triggers")
            except Exception as e:
                print(f"[WARN] Could not drop upload log domain triggers: {e}")
