...
window.loadCategories = async function () {
    ...
    tbody.innerHTML = '<tr><td colspan="5" class="text-center py-8 text-gray-500 text-sm"><i class="fas fa-spinner fa-spin mr-2"></i>Loading categories...</td></tr>';
    ...
    try {
        ...
        if (window.adminSearchQuery) {
            ...
            categories = categories.filter(cat =>
                ...
                (cat.collection_name && cat.collection_name.toLowerCase().includes(query)) ||
                (cat.domains && cat.domains.some(d => (d.name || d).toLowerCase().includes(query)))
            );
        }
        ...
        if (categories.length === 0) {
            tbody.innerHTML = `<tr><td colspan="5" class="text-center py-8 text-gray-500 text-sm">${window.adminSearchQuery ? 'No categories match "' + window.adminSearchQuery + '"' : 'No categories found. Click "Add Category" to create one.'}</td></tr>`;
            return;
        }
        ...
        // Render description as pretty-printed JSON if possible
        const renderDescription = (text) => {
            ...
            if (isJson) {
                return `
                    <div class="relative group">
                        <pre class="text-[10px] leading-tight font-mono bg-gray-50 p-2 rounded border border-gray-200 overflow-y-auto max-h-24 scrollbar-thin scrollbar-thumb-gray-300 whitespace-pre-wrap break-words text-gray-700" title="RAW DATA: ${escapeHtml(text)}">${escapeHtml(displayContent)}</pre>
                        <div class="absolute top-1 right-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            <span class="bg-purple-100 text-purple-700 text-[8px] font-bold px-1.5 py-0.5 rounded shadow-sm">JSON</span>
                        </div>
                    </div>
                `;
            }
            ...
            return `<div class="text-xs text-gray-600 line-clamp-3" title="RAW DATA: ${escapeHtml(text)}">${escapeHtml(text)}</div>`;
        };

        tbody.innerHTML = categories.map(category => {
            const categoryNameEscaped = escapeHtml(category.name || '');
            const descriptionHtml = renderDescription(category.description);
            const collectionNameEscaped = escapeHtml(category.collection_name || '');
            const fullDescription = typeof category.description === 'string' ? category.description : JSON.stringify(category.description || '');
            ...
            <tr class="table-row">
                ...
                <td class="text-sm text-gray-700" style="min-width: 300px; max-width: 450px;">
                    ${descriptionHtml}
                </td>
                <td>
...
window.editCategory = async function (categoryId) {
    ...
    try {
        ...
        // Domains are no longer edited here
...
window.viewCategoryOverview = async function (categoryId) {
    ...
    try {
        ...
        // Full description
        // Full description with JSON pretty-printing
        if (category.description) {
            let descHtml = escapeHtml(category.description);
            try {
                const parsed = JSON.parse(category.description);
                descHtml = `<pre class="text-xs font-mono bg-white p-3 rounded border border-blue-100 overflow-x-auto whitespace-pre-wrap">${escapeHtml(JSON.stringify(parsed, null, 2))}</pre>`;
            } catch (e) {
                descHtml = `<div class="text-sm text-blue-900">${descHtml}</div>`;
            }

            content += `<div class="mt-3 p-3 bg-blue-50 border border-blue-200 rounded-lg">`;
            content += `<div class="text-xs font-semibold text-blue-700 mb-2 flex items-center justify-between">
                            <span>Description</span>
                            <span class="bg-blue-200 text-blue-800 px-1.5 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider">AI Generated</span>
                        </div>`;
            content += descHtml;
            content += `</div>`;
        }
