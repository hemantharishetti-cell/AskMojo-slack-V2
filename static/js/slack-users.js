...
async function loadSlackUsers() {
    ...
    try {
        ...
        let users = await client.getSlackUsers();
        ...
        if (window.adminSearchQuery) {
            ...
            users = users.filter(user =>
                ...
                (user.slack_user_id && user.slack_user_id.toLowerCase().includes(query))
            );
        }

        console.log('Slack users filtered:', users);

        if (!users || users.length === 0) {
            tbody.innerHTML = `<tr><td colspan="6" class="text-center py-8 text-gray-500 text-sm">${window.adminSearchQuery ? 'No Slack users match "' + window.adminSearchQuery + '"' : 'No registered Slack users'}</td></tr>`;
            return;
        }
